...
if ($product_id) {
    ...
    // Fetch additional specifications from tbl_productspecifications
    $additional_specs_sql = "SELECT * FROM tbl_productspecifications WHERE product_id = $product_id";
    $additional_specs_result = $conn->query($additional_specs_sql);
    $additional_specs = [];
    $product_model = ''; // Initialize model variable
    
    while($row = $additional_specs_result->fetch_assoc()) {
        $additional_specs[] = $row;
        // Check if this is the model specification
...
elseif ($category) {
    // Fetch products in the selected category with their model from tbl_productspecifications
    $sql = "SELECT 
                p.product_id,
                p.price,
                p.stock,
            ...
            LEFT JOIN tbl_category c ON p.category_id = c.category_id
            LEFT JOIN tbl_brand b ON p.brand_id = b.brand_id
            LEFT JOIN tbl_productspecifications ps ON p.product_id = ps.product_id AND ps.model = 'Model'
...
<div class="row">
    ...
    <div class="col-md-6">
        ...
        <!-- Main heading shows only the model -->
    ...
    <button type="submit" name="add_to_cart" class="btn btn-primary btn-cart mt-3" <?= $product['stock'] <= 0 ? 'disabled' : '' ?>>
        ...
        <!-- Display the description in a separate section -->
                    ...
                    <div class="row">
                        ...
                        if (isset($result) && $result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo '<a href="?product_id='.$row['product_id'].'">';
                                ...
                                // Model (from product specifications) as title
                                echo '<a href="?product_id='.$row['product_id'].'" class="product-title">';
                                ...
                                // Add to cart button
                                echo '<button class="btn btn-primary btn-cart mt-auto" '.($row['stock'] <= 0 ? 'disabled' : '').'>';
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li><a href="Parts_Contact.php" class="text-white">Contact</a></li>
                    </ul>
                </div>
                <div class="col-md-4">
                    <h5>Contact Us</h5>
