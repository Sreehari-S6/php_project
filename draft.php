...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['component_type'])) {
    ...
    $baseSql = "SELECT p.*, ps.model FROM tbl_products p 
...
// Fetch all processors with their detailed specs and model
$processors = $conn->query("
    SELECT p.*, pr.*, ps.model 
    FROM tbl_products p
                    ...
                    <div class="tab-pane fade <?= $_SESSION['pc_build']['processor'] && !$_SESSION['pc_build']['motherboard'] ? 'show active' : '' ?>" id="motherboard" role="tabpanel">
                        ...
                        <?php if ($_SESSION['pc_build']['processor']): ?>
                            <?php if ($motherboards && $motherboards->num_rows > 0): ?>
                                <p>Compatible with <?= htmlspecialchars($_SESSION['pc_build']['processor']['model']) ?> (Supports <?= htmlspecialchars($_SESSION['pc_build']['processor']['supported_memory_types']) ?>)</p>
                                <div class="row">
                ...
                <div class="build-summary">
                    ...
                    <div class="d-grid gap-2">
                        <?php if (count(array_filter($_SESSION['pc_build'])) === 5): ?>
                            <button class="btn btn-success">Complete Build</button>
                        <?php endif; ?>
